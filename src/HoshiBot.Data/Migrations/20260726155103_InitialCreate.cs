...
namespace HoshiBot.Data.Migrations
{
    ...
    public partial class InitialCreate : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // ---- Hand-added: schema not represented in the EF model (carried over from the
            // ---- pre-squash migration history; see docs/refactoring-plan.md Phase 1).

            // Schema for Quartz.NET's ADO persistent store (JobStoreTX) — verbatim from the official
            // database/tables/tables_postgres.sql (Quartz.NET v3.18.2). Not EF-managed entities; this
            // is raw DDL run through the normal migration pipeline. Table names are lowercase and
            // unquoted; Quartz's default TablePrefix "QRTZ_" resolves to them because Postgres
            // case-folds both the created names and Quartz's unquoted queries to lowercase.
            // (Originally migration 20260718160802_AddQuartzTables.)
            ...
            // connection inherits it. Uses current_database() (via a DO block, since ALTER DATABASE
            // takes only a name literal) so the migration also works on non-default database names —
            // the pre-squash original (20260724115017_AddAiChatEmbeddingHnswIndex) hardcoded
            // "hoshibot". hnsw.* is a dotted custom GUC class, so ALTER DATABASE SET is accepted
            // even before the extension library loads in this session.
            migrationBuilder.Sql("""
                DO $$
                BEGIN
