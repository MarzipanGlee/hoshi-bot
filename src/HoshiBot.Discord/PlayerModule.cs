...
public class PlayerModule(HoshiBotDbContext db) : ApplicationCommandModule<ApplicationCommandContext>
{
    ...
    public async Task<InteractionMessageProperties> LinkPlayer(string playerName, string serverName)
    {
        var userId = Context.User.Id;

        var server = await db.StfcServers.FirstOrDefaultAsync(s => s.Name == serverName);
        if (server is null)
            return EphemeralReply.Of($"No server named \"{serverName}\" found. Ask an admin to add it first (via the web admin).");
        ...
        return EphemeralReply.Of($"Linked your Discord account to **{playerName}** on {server.Name}.");
    }

    [SlashCommand("set-my-alliance", "Set the alliance for your main linked player",
        Contexts = [InteractionContextType.Guild])]
    public async Task<InteractionMessageProperties> SetMyAlliance(string allianceTag)
    {
        var userId = Context.User.Id;
        ...
        if (mainLink is null)
            return EphemeralReply.Of("You haven't linked a player yet. Use /link-player first.");

        var alliance = await db.StfcAlliances.FirstOrDefaultAsync(a =>
            a.ServerId == mainLink.StfcPlayer.ServerId && a.Tag == allianceTag);
        if (alliance is null)
            return EphemeralReply.Of($"No alliance with tag \"{allianceTag}\" found on your server. Ask an admin to add it first (via the web admin).");

        mainLink.StfcPlayer.AllianceId = alliance.Id;
        await db.SaveChangesAsync();

        return EphemeralReply.Of($"Set your alliance to {alliance.Name} ({alliance.Tag}).");
    }
    ...
    public async Task<InteractionMessageProperties> NicknameSync(bool enabled)
    {
        ...
        return EphemeralReply.Of(enabled
