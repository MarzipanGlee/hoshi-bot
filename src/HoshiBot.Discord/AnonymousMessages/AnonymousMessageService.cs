...
public class AnonymousMessageService(HoshiBotDbContext db, GatewayClient gatewayClient, NotificationDispatcher dispatcher, EmbedBranding embedBranding)
{
    public async Task<string> SendAsync(ulong guildId, string subject, string message)
    {
        var settings = await db.GuildSettings.FindAsync(guildId);
        if (settings?.AnonymousMessagesChannelId is not { } channelId)
