...
```bash
docker compose exec ollama ollama pull llama3.1:8b     # chat (Ollama provider) + gate
...
search (knowledge retrieval falls back to keyword full-text search only). `Ollama__GateModel`
enables the passive-listening gate — a YES/NO pre-screen so the bot doesn't run a full answer on
every non-question in a listen channel; left unset the gate is off for Ollama guilds (Gemini guilds
default to a flash-lite gate). It defaults to `llama3.1:8b` (reusing the already-pulled/resident
chat model — the gate prompt is tiny, so it's cheap and needs no extra model); a tiny 1b model was
too weak for reliable German classification. For GPU acceleration, uncomment the `deploy` block on
the `ollama` service.
